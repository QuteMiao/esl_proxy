/* aicpu_runtime.c — onboard AICPU orchestration (CANN entry + cutter/dispatch/orch).
 * Necessary: init_once/platform, role switch, stats, ORCH_CASE.
 * Trimmed unused externs/includes (PR #13). */
#define _GNU_SOURCE
...
int32_t esl_aicpu_execute(EslRuntime *runtime) {
    ...
    LOG_INFO_V0("AICPU thread %d enter execute", idx);
...
__attribute__((visibility("default"))) int esl_aicpu_exec(void *arg) {
    ...
    LOG_INFO_V0("esl_aicpu_exec: start");
    rc = esl_aicpu_execute(runtime);
    LOG_INFO_V0("esl_aicpu_exec: rc=%d", rc);
    if (rc != 0) {
