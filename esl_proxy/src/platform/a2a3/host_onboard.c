...
int esl_onboard_run(int argc, char **argv)
{
    ...
    payload_bytes = (size_t)ESL_PROXY_ONBOARD_WORKER_COUNT * 2U * sizeof(struct task_desc);
    ...
    for (i = 0; i < ESL_PROXY_ONBOARD_WORKER_COUNT; ++i) {
        uint8_t *base = (uint8_t *)dev_payload.ptr +
                        (size_t)i * 2U * sizeof(struct task_desc);
    ...
    {
        EslRuntime host_rt_dump;
        memset(&host_rt_dump, 0, sizeof(host_rt_dump));
        if (aclrtMemcpy(&host_rt_dump, sizeof(host_rt_dump), dev_runtime.ptr,
                sizeof(host_rt_dump), ACL_MEMCPY_DEVICE_TO_HOST) == ACL_SUCCESS) {
            printf("[diag] sched_ts samples=%llu sum_delta_ns=%llu (block0 batch dump)\n",
                (unsigned long long)host_rt_dump.task_window_size,
                (unsigned long long)host_rt_dump.heap_size);
        }
    }
    fflush(stdout);
